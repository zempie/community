<template>
  <router-link :to="`/channel/${portfolio.user_uid}/portfolio/timeline`" class="product-preview">
    <figure
      class="product-preview-image liquid"
      :style="`background: url(${portfolio.thumbnail_img}) center center / cover no-repeat;`"
    >
      <img :src="portfolio.thumbnail_img" alt="item-01" style="display: none" />
    </figure>
    ...
    <div class="product-preview-meta">
      ...
      <div class="product-preview-author">
        <p class="product-preview-author-text">{{ portfolio.created_at }}</p>
      </div>
    </div>
  </router-link>
</template>
