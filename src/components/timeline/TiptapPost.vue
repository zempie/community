...
export default class TiptapPost extends Vue {
  ...
  async created() {
    this.editor = new Editor({
      ...
      onUpdate: () => {
        ...
        // todo: array
        this.checkContent(this.editor.getJSON().content);
  ...
  async checkContent(content: []) {
    ...
    content.forEach((elem: any) => {
      if (elem.type === "image") {
        // const file = dataURItoBlob(elem.attrs.src);
        // console.log(file);
        // console.log(imgFile.size)

        // let file = new File(imgFile, '', {type})

        // this.fileLoader.checkImgFile(imgFile);
        console.log(elem);
      }
      // console.log(elem.)
    });
